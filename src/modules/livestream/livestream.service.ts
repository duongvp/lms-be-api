...
import { calendarTimeNotification } from './calendar-time-notification';
import { Prisma } from '@prisma/client';
import prisma from '../../lib/prisma';
import { resolveCalendarExportSystem } from './calendar-export-system';
import { assistantExportNames, uniqueExportIds } from './calendar-export-fields';
...
const hmoSectionsByPackageCourseCache = new Map<string, {
  expiresAt: number;
  data: HocmaiSectionOption[];
}>();
const HMO_SECTIONS_CACHE_TTL_MS = 5 * 60 * 1000;

const packageCourseKey = (packageId: unknown, courseId: unknown) => (
  JSON.stringify([String(packageId), String(courseId)])
);
...
const normalizeTeachingAssignmentsForScheduleUpdate = (data: any) => {
  const hasTeacher = Object.prototype.hasOwnProperty.call(data, 'teacher');
  const hasAssistants = Object.prototype.hasOwnProperty.call(data, 'assistant_teacher');
...
const createCalendarRecord = async (
  ...
  prismaData.evg_banner = await resolveProgramTeacherBanner(
    client,
    prismaData.code,
    prismaData.teacher
  );
...
const updateCalendarRecord = async (
  ...
  if (!preloadedBefore) await hydrateAssistantTeachers(client, [before]);
  const hasAssistants = Object.prototype.hasOwnProperty.call(data, 'assistant_teacher');
  ...
  if (bannerIdentityChanged) {
    prismaData.evg_banner = await resolveProgramTeacherBanner(client, nextCode, nextTeacher);
  }
...
const calendarSubjectName = (subject: unknown, grade: unknown, systemType: unknown) => {
  if (systemType !== 'topclass') return subject;
  const name = typeof subject === 'string' ? subject.trim() : '';
  const block = Number(grade);
  if (!name || !Number.isInteger(block) || block < 1 || block > 12) return subject;
  // Một số dữ liệu cũ đã có khối trong tên môn; không thêm lần thứ hai.
  return name.endsWith(` ${block}`) ? name : `${name} ${block}`;
};

const hydrateLessonData = async (
  tx: any,
  input: any,
  lessonCache?: Map<string, any>
) => {
  const data = { ...input };
...
const ensureNotBeforeNow = (value: Date) => {
...
const copyPackageLessonMappingsForCalendar = async (
  tx: Prisma.TransactionClient,
  calendar: any,
  mappings: Array<{
    package_id: string;
...
const getRescheduleLessonNameOptions = (payload: any) => ({
  canceledLessonName: (lessonName: unknown) => formatRescheduledLessonName(
    lessonName,
    normalizeLessonNameAffix(
      payload?.canceled_lesson_name_prefix,
      DEFAULT_CANCELED_LESSON_NAME_PREFIX,
      'Tiền tố tên bài nghỉ'
    ),
    normalizeLessonNameAffix(
      payload?.canceled_lesson_name_suffix,
      '',
      'Hậu tố tên bài nghỉ'
    )
  ),
  makeupLessonName: (lessonName: unknown) => formatRescheduledLessonName(
    lessonName,
    normalizeLessonNameAffix(
      payload?.new_lesson_name_prefix,
      DEFAULT_MAKEUP_LESSON_NAME_PREFIX,
      'Tiền tố tên bài mới'
    ),
    normalizeLessonNameAffix(
      payload?.new_lesson_name_suffix,
      '',
      'Hậu tố tên bài mới'
    )
  ),
...
const normalizeHocmaiBatchLessonIds = (lessonIds: unknown): string[] => {
...
export const getHocmaiSectionsForProgramLessons = async (
  programCode: string,
  lessonIds: unknown
): Promise<{
  ...
  const code = String(programCode || '').trim();
  if (!code) throw new Error('Chương trình không hợp lệ');
  ...
  const byLessonId: Record<string, HocmaiSectionOption[]> = Object.fromEntries(
    normalizedLessonIds.map((lessonId) => [lessonId, []])
  );
  ...
  const mappings = await prisma.$queryRaw<Array<{
    ...
    WHERE lesson.id IN (${Prisma.join(parsedLessonIds)})
  ...
  const pairKeysByLessonId = new Map<string, Set<string>>();
  const uniquePairs = new Map<string, { packageId: string; courseId: string }>();
  mappings.forEach((mapping) => {
    ...
    const lessonPairs = pairKeysByLessonId.get(lessonId) || new Set<string>();
    lessonPairs.add(pairKey);
    pairKeysByLessonId.set(lessonId, lessonPairs);
  });

  const optionsByPair = new Map<string, HocmaiSectionOption[]>();
  const errorsByPair = new Map<string, string>();
  const missingPairs: Array<{ packageId: string; courseId: string }> = [];
  const now = Date.now();
  uniquePairs.forEach((pair, pairKey) => {
    ...
    if (cached) hmoSectionsByPackageCourseCache.delete(pairKey);
  ...
  if (missingPairs.length) {
    ...
    await Promise.all(Array.from(
      { length: Math.min(concurrency, missingPairs.length) },
      () => worker()
    ));
  }

  const errorsByLessonId: Record<string, string[]> = {};
  pairKeysByLessonId.forEach((pairKeys, lessonId) => {
    const uniqueOptions = new Map<string, HocmaiSectionOption>();
    const lessonErrors: string[] = [];
    pairKeys.forEach((pairKey) => {
      const pairError = errorsByPair.get(pairKey);
      if (pairError) lessonErrors.push(pairError);
    ...
    if (lessonErrors.length) errorsByLessonId[lessonId] = Array.from(new Set(lessonErrors));
  });
...
export const getHocmaiSectionsForProgramLesson = async (
  ...
  const result = await getHocmaiSectionsForProgramLessons(programCode, [normalizedLessonId]);
...
export const createBulk = async (config: any, changeActor?: CalendarChangeActor) => {
  ...
  return await withCalendarTriggerErrorHint(() => prisma.$transaction(async (tx) => withManualHocmaiQueue(tx, async () => {
    ...
    const sessionIds = Array.from(new Set(calendars.flatMap((calendar: any) => {
      const rawSessionId = calendar?.session_id ?? calendar?.sessionId;
      if (rawSessionId === undefined || rawSessionId === null || rawSessionId === '') return [];
      try {
        return [BigInt(rawSessionId)];
      } catch {
        return [];
      }
    }).map((sessionId: bigint) => sessionId.toString()))).map((sessionId) => BigInt(sessionId));
    ...
    const lessonCache = new Map(
      lessons.map((lesson: any) => [String(lesson.id), lesson])
    );
    ...
    await enqueueManyCalendarTeamsNotifications(tx, teamsEvents);
    ...
    async (tx) => withManualHocmaiQueue(tx, async () => {
      ...
      const prepared = await Promise.all(rows.map(async (row) => {
        ...
        return {
          ...
          end_time: endTime,
          lesson_count: lessonCount,
          evg_banner: await resolveProgramTeacherBanner(tx, calendar.code, calendar.teacher),
          key: generateKey(
            calendar.system_type,
            startTime,
            calendar.code,
  ...
  return withCalendarTriggerErrorHint(() => withSerializableTransaction(
    async (tx) => withManualHocmaiQueue(tx, async () => {
      ...
      const sourceMappingsByKey = operation === 'makeup'
        ? await loadMappingsByKeys(
            tx,
            selected.map((session) => String(session.key || '').trim()).filter(Boolean)
          )
        : new Map<string, any[]>();
...
export const updateSchedule = async (
  ...
  const timeNotification = calendarTimeNotification(current, data, sendTimeNotification, auditReason);
  if (timeNotification !== undefined) data.lesson_noti = timeNotification;
...
export const getCalendar = async (
  ...
  const requestedTeachers = Array.from(new Set(
    String(query.teacher ?? '')
      .split(',')
  ...
  if (teacherNames.length) {
    const teacherCondition: Prisma.calendarWhereInput = {
  ...
  if (weekdays.length && weekdays.length < 7) {
    ...
    const weekdayRows = await prisma.$queryRaw<Array<{ id: number }>>(Prisma.sql`
      SELECT id
      FROM calendar
      WHERE WEEKDAY(start_time) + 1 IN (${Prisma.join(weekdays)})
    `);
    where.AND = [
      ...(Array.isArray(where.AND) ? where.AND : []),
  ...
  const assignmentStatuses = data.length
    ...
    : [];
  const assignmentStatusByCalendarId = new Map(
    assignmentStatuses.map((status) => [status.calendar_id, status._max.created_at])
  );
...
export const getCalendarRowsForExport = async (
  ...
  const exportLessons = await prisma.lessons.findMany({
  ...
  const exportPrograms = new Map<string, typeof exportLessons>();
  for (const lesson of exportLessons) exportPrograms.set(lesson.subject_code, [...(exportPrograms.get(lesson.subject_code) || []), lesson]);
  const exportCourseMappings = exportLessons.length ? await prisma.lesson_course_mapping.findMany({
  ...
  const exportCoursesByLesson = new Map<string, typeof exportCourseMappings>();
  ...
  const formatDirectDateTime = (value: Date) => [
  ...
  return calendars.map((calendar) => {
    ...
    const exportSystem = resolveCalendarExportSystem(calendar, linkedProgram, programCandidates);
    ...
    return {
      ...
      content_homework: '',
      assistant_name: assistantExportNames(assistants, displayNameByUsername),
      sharepoint_link: calendar.lesson_link || '',
      course_ids: uniqueMappingValues(calendarMappings, 'course_id') || uniqueExportIds(lessonMappings.map((mapping) => mapping.course_id)),
      lesson_ids: uniqueMappingValues(calendarMappings, 'lesson_id'),
      package_ids: uniqueMappingValues(calendarMappings, 'package_id') || uniqueExportIds(lessonMappings.map((mapping) => mapping.package_id)),
...
export const updateBulk = async (
  ...
  if (config_mode === 'common') {
    ...
    const hasCommonCalendarFieldUpdate = Object.keys(dataToUpdate).some(
      (key) => key !== 'skip_teacher_user_sync'
    );
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      const applyUpdates = async () => {
        for (const item of update_data) {
          ...
          const hasCalendarFieldUpdate = Object.keys(dataToUpdate).some(
            (key) => key !== 'skip_teacher_user_sync'
          );
...
export const backfillMissingCalendarTeachingUsers = async (ids?: number[], additionalUsers?: unknown) => {
  ...
  if (ids && ids.length > 0) {
    ...
    while (true) {
      ...
      if (batchResult.lastId == null) break;
