...
const buildCalendarPayload = (
  calendar: any,
  mappings: Array<{ package_id: unknown; lesson_id: unknown }>,
...
export type CalendarPackageLessonMapping = {
  package_id: string;
  course_id: string;
  lesson_id: string;
};
 ...
 */
export const reconcileCalendarMappingsAndEnqueue = async (
  tx: TransactionClient,
  calendar: any,
  desiredMappings: CalendarPackageLessonMapping[],
  operationId = crypto.randomUUID(),
  sequenceNo = 1,
  enqueueFullPayload = true,
  allowEmptyDesiredMappings = false
) => {
  ...
  const programCode = String(
    calendar?.code ?? calendar?.program_code ?? calendar?.subject_code ?? ''
  ).trim();
  ...
  if (!key) throw new Error(`Lịch ${calendarId} chưa có key để đồng bộ mapping HMO`);
  ...
  const desiredByPair = new Map<string, CalendarPackageLessonMapping>();
  ...
  const currentByPair = new Map<string, typeof current>();
  ...
  const retainedByPair = new Map<string, (typeof current)[number]>();
  desiredByPair.forEach((desired, pairKey) => {
    ...
    const retained = candidates.find((item) => String(item.lesson_id) === desired.lesson_id)
      || candidates[0];
    if (retained) retainedByPair.set(pairKey, retained);
  });
  ...
  for (const [pairKey, desired] of desiredByPair) {
    ...
    if (
      String(retained.lesson_id) !== desired.lesson_id
 ...
 */
export const enqueueCalendarsSyncBulk = async (
  tx: TransactionClient,
  calendarIds: number[],
  operationId = crypto.randomUUID()
) => {
  const ids = Array.from(new Set(calendarIds));
  if (!ids.length) return { operationId, queuedIds: [], skippedIds: [], missingIds: [] };

  const calendars = await tx.$queryRaw<any[]>(Prisma.sql`
    SELECT
      id,
      \`key\`,
      code,
  ...
  const mappings = keys.length
    ...
    : [];
  const mappingsByKey = new Map<string, Array<{ package_id: string; lesson_id: string }>>();
  ...
  const rows: Prisma.hocmai_sync_queueCreateManyInput[] = [];
  ...
  if (rows.length) await tx.hocmai_sync_queue.createMany({ data: rows });
...
export const withManualHocmaiQueue = async <T>(
  ...
  try {
    ...
    try {
      ...
      if (error?.code !== 'P2028') throw error;
    }
  }
};
