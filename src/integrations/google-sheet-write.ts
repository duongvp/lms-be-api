...
export const DEFAULT_CALENDAR_SHEET_URL = 'https://docs.google.com/spreadsheets/d/1NTrAeL-tEykqqm9R_kQxHVBS-xaGNgarsQ9xP-HM774/edit';
export const DEFAULT_TOPUNI_SHEET_URL = 'https://docs.google.com/spreadsheets/d/1DejptU8MU5Zvey_6Z5jtFHlwjGIgKpe3Woqq01IjJrw/edit';

export const spreadsheetIdFromUrl = (value: string) => {
  let url: URL;
  try { url = new URL(value); } catch { throw new Error('Link Google Sheets không hợp lệ.'); }
...
let cachedAuth: InstanceType<typeof google.auth.JWT> | undefined;

export const normalizeSheetPrivateKey = (value: string) => value.trim()
  .replace(/^["']+|["']+$/g, '')
  .replace(/\\+r\\+n/g, '\n')
  .replace(/\\+n/g, '\n')
  .replace(/\r\n/g, '\n')
...
const sheetsClient = () => {
  ...
  const key = normalizeSheetPrivateKey(process.env.GOOGLE_SHEETS_PRIVATE_KEY || '');
  if (!email || !key) throw new Error('Chưa cấu hình tài khoản Google Sheets trên backend. Cần GOOGLE_SHEETS_CLIENT_EMAIL và GOOGLE_SHEETS_PRIVATE_KEY.');
  if (!cachedAuth || cachedAuth.email !== email || cachedAuth.key !== key) {
    try {
      if (createPrivateKey(key).asymmetricKeyType !== 'rsa') throw new Error('Not RSA');
    } catch {
...
export const workbookRequests = (buffer: Buffer, suffix: string): sheets_v4.Schema$Request[] => {
  const workbook = XLSX.read(buffer, { type: 'buffer' });
  const used = new Set<number>();
  return workbook.SheetNames.flatMap((name) => {
    let sheetId: number;
    do { sheetId = randomInt(1, 2_000_000_000); } while (used.has(sheetId));
    used.add(sheetId);
    const matrix = XLSX.utils.sheet_to_json<any[]>(workbook.Sheets[name], { header: 1, defval: '' });
...
export const calendarSessionEnded = (date: string, timeRange: string, now: Date) => {
  const day = date.match(/^(\d{2})\/(\d{2})\/(\d{4})$/);
  const times = [...timeRange.matchAll(/(\d{2}):(\d{2})/g)];
  if (!day || times.length !== 2) return false;
  ...
  if (times.some((time) => Number(time[1]) > 23 || Number(time[2]) > 59)) return false;
...
export const calendarSheetFormatting = (requests: sheets_v4.Schema$Request[], now = new Date()): sheets_v4.Schema$Request[] => {
  const color = (hex: string) => ({ red: parseInt(hex.slice(0, 2), 16) / 255, green: parseInt(hex.slice(2, 4), 16) / 255, blue: parseInt(hex.slice(4, 6), 16) / 255 });
  const result: sheets_v4.Schema$Request[] = [];
  for (const request of requests) {
    if (!request.updateCells?.rows?.length) continue;
    ...
    result.push(
      { repeatCell: { range, cell: { userEnteredFormat: { backgroundColor: color('FFFFFF'), textFormat: { fontFamily: 'Arial', fontSize: 10, foregroundColor: color('000000') }, verticalAlignment: 'MIDDLE', wrapStrategy: 'CLIP' } }, fields: 'userEnteredFormat' } },
      { repeatCell: { range: { ...range, endRowIndex: 1 }, cell: { userEnteredFormat: { backgroundColor: color('0C343D'), textFormat: { bold: true, foregroundColor: color('FFFFFF') }, wrapStrategy: 'WRAP' } }, fields: 'userEnteredFormat.backgroundColor,userEnteredFormat.textFormat.bold,userEnteredFormat.textFormat.foregroundColor,userEnteredFormat.wrapStrategy' } },
      { updateBorders: { range, top: { style: 'SOLID', color: color('00B050') }, bottom: { style: 'SOLID', color: color('00B050') }, left: { style: 'SOLID', color: color('00B050') }, right: { style: 'SOLID', color: color('00B050') }, innerHorizontal: { style: 'SOLID', color: color('00B050') }, innerVertical: { style: 'SOLID', color: color('00B050') } } },
      { updateDimensionProperties: { range: { sheetId, dimension: 'ROWS', startIndex: 0, endIndex: 1 }, properties: { pixelSize: 36 }, fields: 'pixelSize' } },
      { updateSheetProperties: { properties: { sheetId, gridProperties: { frozenRowCount: 1, frozenColumnCount: Math.min(3, headers.length) } }, fields: 'gridProperties.frozenRowCount,gridProperties.frozenColumnCount' } },
    );
    headers.forEach((header, index) => {
      ...
      if (header.startsWith('Tài liệu live') || header === 'Nhiệm vụ học tập') result.push({ repeatCell: { range: { ...range, endRowIndex: 1, startColumnIndex: index, endColumnIndex: index + 1 }, cell: { userEnteredFormat: { backgroundColor: color('741B47') } }, fields: 'userEnteredFormat.backgroundColor' } });
    });
    ...
    let runStart = -1;
    const flush = (endRowIndex: number) => {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      const ended = dateColumn >= 0 && timeColumn >= 0 && calendarSessionEnded(String(values[dateColumn]?.userEnteredValue?.stringValue || ''), String(values[timeColumn]?.userEnteredValue?.stringValue || ''), now);
      if (ended && runStart < 0) runStart = i;
      if (!ended) flush(i);
    }
    flush(rows.length);
    // Nghỉ học: màu chữ đỏ trên toàn dòng, giữ nền xanh nếu đã qua ngày.
    let canceledStart = -1;
    const flushCanceled = (endRowIndex: number) => {
      if (canceledStart < 0) return;
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (canceled && canceledStart < 0) canceledStart = i;
      if (!canceled) flushCanceled(i);
    }
    flushCanceled(rows.length);
  }
  return result;
};
const requestBytes = (request: sheets_v4.Schema$Request) => Buffer.byteLength(JSON.stringify(request));

// Measure UTF-8 bytes, including JSON overhead, rather than relying on row counts.
export const splitSheetRequests = (requests: sheets_v4.Schema$Request[], maxBytes = MAX_BATCH_BYTES) => {
  const fragments: sheets_v4.Schema$Request[] = [];
  for (const request of requests) {
    ...
    let size = 1024;
    const flush = () => {
      if (rows.length) fragments.push({ updateCells: { ...update, start: { ...base, rowIndex: startRow }, rows } });
    ...
    for (const row of update.rows || []) {
      ...
      if (size + rowBytes > maxBytes) flush();
      if (rowBytes + 1024 > maxBytes) {
        let values: sheets_v4.Schema$CellData[] = [];
        let columnIndex = base.columnIndex || 0;
        let cellSize = 1024;
        const flushCells = () => {
          if (values.length) fragments.push({ updateCells: { ...update, start: { ...base, rowIndex: startRow, columnIndex }, rows: [{ values }] } });
        ...
        for (const cell of row.values || []) {
          ...
          if (bytes + 1024 > maxBytes) throw new Error('Một ô dữ liệu vượt kích thước Google Sheets có thể nhận. Hãy kiểm tra nội dung ô.');
          if (cellSize + bytes > maxBytes) flushCells();
          values.push(cell);
          cellSize += bytes;
        }
        flushCells();
  ...
  let batch: sheets_v4.Schema$Request[] = [];
  let bytes = 32;
  for (const request of fragments) {
    const size = requestBytes(request) + 1;
    if (size + 32 > maxBytes) throw new Error('Một yêu cầu Google Sheets vượt kích thước cho phép.');
    if (bytes + size > maxBytes || batch.length >= 100) { batches.push(batch); batch = []; bytes = 32; }
    batch.push(request);
    bytes += size;
  }
...
let nextWriteAt = 0;
const pacedWrite = <T>(task: () => Promise<T>): Promise<T> => {
...
const errorStatus = (error: unknown) => {
  const failure = error as { response?: { status?: number }; code?: string | number };
...
export const readSheetMetadata = async (sheets: sheets_v4.Sheets, spreadsheetId: string) => {
  for (let attempt = 0; ; attempt++) {
    try {
      ...
      if ((status && status !== 408 && status !== 429 && status < 500) || attempt >= 3) {
        if (status === 403 || status === 404) throw new Error('Không thể truy cập sheet. Kiểm tra quyền Chỉnh sửa của tài khoản dịch vụ.');
...
const failureMessage = (error: unknown) => {
  const failure = error as { code?: string | number; cause?: { code?: string | number }; response?: { data?: { error?: string } } };
  const reason = failure.response?.data?.error;
  if (reason === 'invalid_grant' || reason === 'invalid_client') return 'Google không chấp nhận tài khoản dịch vụ. Kiểm tra khóa JSON còn hiệu lực và client_email khớp với private_key trong cùng file.';
  const code = failure.cause?.code || failure.code;
  if (code === 'ERR_OSSL_UNSUPPORTED' || (typeof code === 'string' && code.startsWith('ERR_OSSL_PEM'))) return 'Không thể đọc GOOGLE_SHEETS_PRIVATE_KEY. Kiểm tra định dạng khóa trong cấu hình backend.';
  if (code === 'ENOTFOUND' || code === 'EAI_AGAIN') return 'Backend không phân giải được địa chỉ Google. Kiểm tra DNS và kết nối mạng của máy chủ.';
  if (code === 'CERT_HAS_EXPIRED' || code === 'UNABLE_TO_VERIFY_LEAF_SIGNATURE' || code === 'SELF_SIGNED_CERT_IN_CHAIN') return 'Backend không xác minh được chứng chỉ HTTPS của Google. Kiểm tra chứng chỉ và proxy của máy chủ.';
  return undefined;
};

export const validateSheetExport = (sheetUrl: string) => {
  spreadsheetIdFromUrl(sheetUrl);
  sheetsClient();
};

export const replacementSheetRequests = (additions: sheets_v4.Schema$Request[], existing: sheets_v4.Schema$Sheet[], suffix: string, finalNames?: Map<number, string>): sheets_v4.Schema$Request[] => {
...
export const writeCalendarWorkbookToSheet = async (buffer: Buffer, sheetUrl: string, onProgress?: (progress: number, message: string) => void, tabTitles?: string[]) => {
  const spreadsheetId = spreadsheetIdFromUrl(sheetUrl);
  const suffix = `LMS ${new Date().toISOString().replace(/[:.]/g, '-')}-${randomInt(1000, 9999)}`;
  const requests = workbookRequests(buffer, suffix);
  const sheets = sheetsClient();
  onProgress?.(0, 'Đang kết nối Google Sheets và đọc danh sách tab…');
  const existing = await readSheetMetadata(sheets, spreadsheetId);
  ...
  const creationBatches = splitSheetRequests(additions);
  const dataBatches = splitSheetRequests(requests.filter((request) => request.updateCells));
  const formattingBatches = splitSheetRequests(calendarSheetFormatting(requests));
  const replacement = replacementSheetRequests(additions, existing.data.sheets || [], suffix, finalNames);
  // Delete old tabs and rename new ones together; retain old data while uploading.
  if (Buffer.byteLength(JSON.stringify({ requests: replacement })) > MAX_BATCH_BYTES) throw new Error('Số tab quá lớn để thay thế trong một lần.');
  const attempted = new Map<number, string>();
  let committed = 0;
  const total = creationBatches.length + dataBatches.length + formattingBatches.length + 1;
  const send = async (batch: sheets_v4.Schema$Request[], safeToRetry: boolean) => {
    for (let attempt = 0; ; attempt++) {
      try {
        ...
        if (batch.every((request) => request.addSheet) && (!status || status === 408 || status >= 500 || status === 400)) {
          const current = await readSheetMetadata(sheets, spreadsheetId);
          if (batch.every((request) => current.data.sheets?.some((sheet) => sheet.properties?.sheetId === request.addSheet!.properties!.sheetId && sheet.properties?.title === request.addSheet!.properties!.title))) break;
          ...
          if (status !== 400 && attempt < 5) { await pause(1000 * 2 ** attempt); continue; }
        }
        ...
        await pause(Math.min(32_000, 1000 * 2 ** attempt) + randomInt(0, 1000));
      }
    }
    committed++;
  ...
  try {
    for (const batch of creationBatches) {
      for (const request of batch) attempted.set(request.addSheet!.properties!.sheetId!, request.addSheet!.properties!.title!);
      await send(batch, false);
    }
    for (const batch of dataBatches) await send(batch, true);
    for (const batch of formattingBatches) await send(batch, true);
    try {
      ...
      const current = await readSheetMetadata(sheets, spreadsheetId);
      const complete = additions.every((request) => current.data.sheets?.some((sheet) =>
        sheet.properties?.sheetId === request.addSheet.properties.sheetId &&
    ...
    if (attempted.size) {
      try {
        ...
        for (const batch of splitSheetRequests(deletions)) await pacedWrite(() => sheets.spreadsheets.batchUpdate({ spreadsheetId, requestBody: { requests: batch } }, { timeout: WRITE_TIMEOUT_MS, retry: false }));
      } catch { cleanupFailed = true; }
    }
    const status = errorStatus(error);
    const specificMessage = failureMessage(error);
    if (specificMessage) throw new Error(specificMessage);
    if (status === 403 || status === 404) throw new Error(`Không thể ghi vào sheet. Hãy bật Google Sheets API và chia sẻ quyền Chỉnh sửa cho ${process.env.GOOGLE_SHEETS_CLIENT_EMAIL}.`);
    if (status === 401 || status === 400) throw new Error('Google Sheets từ chối yêu cầu. Kiểm tra cấu hình tài khoản dịch vụ và dữ liệu xuất.');
