...
exports.buildTeamsAdaptiveCard = exports.buildCalendarChanges = void 0;
const TIME_ZONE = 'Asia/Ho_Chi_Minh';
const EMPTY_VALUE = '—';
const asWallClockParts = (value) => {
    if (!value)
        return null;
...
const buildCalendarChanges = (before, after) => {
    if (!before || !after)
        return [];
    const changes = [];
    const add = (label, oldValue, newValue) => {
        if (oldValue !== newValue)
            changes.push({ label, before: oldValue, after: newValue });
    };
    ...
    add('Thời gian', `${formatScheduleTime(before.start_time)} – ${formatScheduleTime(before.end_time)}`, `${formatScheduleTime(after.start_time)} – ${formatScheduleTime(after.end_time)}`);
...
exports.buildTeamsAdaptiveCard = buildTeamsAdaptiveCard;
