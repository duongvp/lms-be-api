...
exports.clearPackageCourseSheetCache = exports.listPackageCoursesFromSheet = exports.resolvePackagesByCourseId = void 0;
const ApiError_1 = __importDefault(require("../utils/ApiError"));
const DEFAULT_SHEET_URL = 'https://docs.google.com/spreadsheets/d/1m_KNVZc5PMu-UQi2PCrEGuXAfVM0NPGEghRbEYnJeH0/edit?usp=sharing';
...
const toCsvExportUrl = (sourceUrl) => {
    let parsed;
    try {
        parsed = new URL(sourceUrl);
    }
    catch {
        throw new ApiError_1.default('PACKAGE_COURSE_SHEET_URL không hợp lệ', 500);
    }
    const spreadsheetId = parsed.pathname.match(/\/spreadsheets\/d\/([^/]+)/)?.[1];
    if (!spreadsheetId) {
        throw new ApiError_1.default('Không xác định được spreadsheet ID từ PACKAGE_COURSE_SHEET_URL', 500);
    }
...
const downloadRows = async () => {
    ...
    if (!response.ok) {
        throw new ApiError_1.default(response.status === 401 || response.status === 403
            ? 'Google Sheet package-course chưa được cấp quyền Viewer cho backend'
            : `Không thể đọc Google Sheet package-course (HTTP ${response.status})`, 503);
    }
    const content = await response.text();
    if (/^\s*<!doctype html/i.test(content)) {
        throw new ApiError_1.default('Google Sheet trả về trang đăng nhập thay vì dữ liệu CSV', 503);
    }
    const csvRows = parseCsv(content);
    if (csvRows.length < 2) {
        throw new ApiError_1.default('Google Sheet package-course không có dữ liệu', 503);
    }
    ...
    if (packageIndex < 0 || courseIndex < 0) {
        throw new ApiError_1.default(`Google Sheet cần có cột package_id và course_id. Các cột hiện tại: ${csvRows[0].join(', ')}`, 503);
    }
    ...
    for (const row of csvRows.slice(1)) {
        ...
        if (!packageId && !courseId)
            continue;
        if (!packageId || !courseId) {
            throw new ApiError_1.default('Google Sheet có dòng thiếu package_id hoặc course_id', 503);
        }
        if (packageId.length > 50 || courseId.length > 50) {
            throw new ApiError_1.default('package_id/course_id trong Google Sheet vượt quá 50 ký tự', 503);
        }
    ...
    if (rowsByCourseId.size === 0) {
        throw new ApiError_1.default('Google Sheet package-course không có dòng hợp lệ', 503);
    }
    return rowsByCourseId;
};
const getRowsByCourseId = async () => {
    if (cache && cache.expiresAt > Date.now())
        return cache.rowsByCourseId;
    try {
...
const resolvePackagesByCourseId = async (courseIdInput) => {
    const courseId = String(courseIdInput ?? '').trim();
    if (!courseId)
        throw new ApiError_1.default('Vui lòng cung cấp course_id', 400);
    if (courseId.length > 50) {
        throw new ApiError_1.default('course_id không được vượt quá 50 ký tự', 400);
    }
    const rows = (await getRowsByCourseId()).get(courseId);
    if (!rows?.length) {
        throw new ApiError_1.default(`Course ID ${courseId} không tồn tại trong Google Sheet`, 400);
    }
    return rows;
};
exports.resolvePackagesByCourseId = resolvePackagesByCourseId;
const listPackageCoursesFromSheet = async () => (Array.from((await getRowsByCourseId()).values())
    .flat()
    .sort((left, right) => (left.course_id.localeCompare(right.course_id, undefined, { numeric: true })
    || left.package_id.localeCompare(right.package_id, undefined, { numeric: true }))));
exports.listPackageCoursesFromSheet = listPackageCoursesFromSheet;
const clearPackageCourseSheetCache = () => {
    cache = null;
    refreshPromise = null;
};
exports.clearPackageCourseSheetCache = clearPackageCourseSheetCache;
