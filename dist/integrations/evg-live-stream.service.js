...
exports.createEvgLiveStream = void 0;
...
const positiveIntegerConfig = (name, fallback) => {
    const value = Number(process.env[name] || fallback);
    if (!Number.isInteger(value) || value <= 0)
        throw new Error(`${name} phải là số nguyên dương`);
    return value;
};
const nonNegativeIntegerConfig = (name, fallback) => {
    const value = Number(process.env[name] ?? fallback);
    if (!Number.isInteger(value) || value < 0)
...
const waitForEvgRequestSlot = async (intervalMs) => {
    const delayMs = Math.max(0, nextEvgRequestAt - Date.now());
    if (delayMs)
        await wait(delayMs);
    nextEvgRequestAt = Date.now() + intervalMs;
};
const retryAfterMilliseconds = (response, retryIndex, baseDelayMs) => {
...
const createEvgLiveStream = async (name) => {
    ...
    if (!response)
        throw new Error('Không nhận được phản hồi từ EVG');
    let raw;
    try {
...
exports.createEvgLiveStream = createEvgLiveStream;
