...
export const buildPublicGoogleSheetCsvUrls = (sheetUrl: string) => {
  ...
  const sheetMatch = parsed.pathname.match(
    /^\/spreadsheets\/(?:u\/\d+\/)?d\/([a-zA-Z0-9_-]+)/
  );
  if (!sheetMatch) throw new Error('Link Google Sheets không hợp lệ');
...
const looksLikeHtml = (contentType: string, buffer: Buffer) => {
...
export const getPublicGoogleSheetCsv = async (sheetUrl: string) => {
  const candidateUrls = buildPublicGoogleSheetCsvUrls(sheetUrl);
  ...
  throw new Error(
    `Không thể xuất tab Google Sheets thành CSV${statuses.length ? ` (HTTP ${statuses.join('/')})` : ''}. `
