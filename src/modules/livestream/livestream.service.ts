...
const normalizeTeachingAssignments = async (client: any, data: any) => {
  ...
  if (assignmentValues.length) {
    const profiles = await client.$queryRaw(Prisma.sql`
      ...
      WHERE (
          username IN (${Prisma.join(assignmentValues)})
          ${teacher ? Prisma.sql`OR display_name = ${teacher}` : Prisma.empty}
        )
        AND status = 1
    `) as Array<{
      username: string;
    ...
    const teacherProfilesByDisplayName = teacher
      ? profiles.filter((profile) => (
          profile.display_name === teacher && profile.teacher_type === 1
        ))
      : [];
  ...
  if (hasTeacher && !teacher) data.teacher = null;
  if (hasAssistants) {
