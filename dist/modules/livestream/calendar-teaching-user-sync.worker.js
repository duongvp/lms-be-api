...
exports.startCalendarTeachingUserSyncWorker = exports.createCalendarTeachingUserSyncCheck = exports.getCalendarTeachingUserSyncStatus = exports.runCalendarTeachingUserSync = exports.calendarTeachingUserSyncWindow = void 0;
...
let timer = null;
const calendarTeachingUserSyncWindow = (now = new Date()) => {
...
exports.calendarTeachingUserSyncWindow = calendarTeachingUserSyncWindow;
...
const runCalendarTeachingUserSync = async (now = new Date()) => {
    const { start, end } = (0, exports.calendarTeachingUserSyncWindow)(now);
  ...
  `;
    ...
    `;
        ...
        if (duplicate)
            return { started: false, message: 'Một lượt quét user nhân sự đang chạy' };
        throw error;
    }
...
exports.runCalendarTeachingUserSync = runCalendarTeachingUserSync;
const getCalendarTeachingUserSyncStatus = async () => {
    let latest = null;
    try {
...
exports.getCalendarTeachingUserSyncStatus = getCalendarTeachingUserSyncStatus;
const createCalendarTeachingUserSyncCheck = (sync = exports.runCalendarTeachingUserSync, now = () => new Date(), onSuccess = () => undefined, onError = () => undefined) => {
    let lastDate = '';
    let running = false;
    return async () => {
        const current = now();
        ...
        if (parts.hour !== '03' || parts.minute !== '00' || lastDate === date || running)
            return;
        running = true;
        try {
...
exports.createCalendarTeachingUserSyncCheck = createCalendarTeachingUserSyncCheck;
const startCalendarTeachingUserSyncWorker = () => {
    if (timer || process.env.CALENDAR_TEACHING_USER_CRON_ENABLED?.toLowerCase() === 'false') {
        return () => undefined;
    }
    const check = (0, exports.createCalendarTeachingUserSyncCheck)(exports.runCalendarTeachingUserSync, () => new Date(), (result) => logger_1.logger.info('Calendar teaching user sync completed', result), (error) => logger_1.logger.error('Calendar teaching user sync failed:', error instanceof Error ? error.message : error));
    void check();
    timer = setInterval(() => void check(), CHECK_INTERVAL_MS);
    timer.unref();
    logger_1.logger.info(`Calendar teaching user sync cron enabled at 03:00 (${TIMEZONE})`);
    return () => {
        if (timer)
            clearInterval(timer);
        timer = null;
...
exports.startCalendarTeachingUserSyncWorker = startCalendarTeachingUserSyncWorker;
