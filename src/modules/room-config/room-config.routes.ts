...
router.post('/import', authorize(['room_config.import', 'room_config.create']), authorizeProgramForAny(['room_config.import', 'room_config.create'], (req) => String(req.body?.program_code || '')), authorizeFields('room_config', () => ['code', 'learn_number', 'config', 'teacher', 'assistant_teacher']), controller.importBulk);
